...
Arch Diff Miner is a Python 3.14 freethreaded (no-GIL) command-line tool that mines architecture decision logs (ADLs) for training data. Powered by `pygit2`/libgit2 and a Typer CLI, it walks a target repository, captures `(intent, code_diffs, adl_diff)` tuples, and emits them into `training_dataset.json` for downstream fine-tuning pipelines.
...
3. Generate tuples with Typer: `uv run python mine_adl_diffs.py --adl-file adl.yaml --output training_dataset.json`.
...
- Run `uv run python mine_adl_diffs.py --help` to view the full Typer help text.
